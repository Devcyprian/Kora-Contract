...
use kora_shared::{
    ...
    types::{Listing, MultisigConfig, RiskTier},
...
pub enum DataKey {
    ...
    /// Referrer recorded at listing time, earns a share of the funding fee.
    Referrer(u64),
    /// Multisig-gated admin proposal by id.
    AdminProposal(u64),
    /// Monotonic counter allocating the next admin proposal id.
    NextAdminProposalId,
}
...
impl MarketplaceContract {
    ...
    /// Update the marketplace fee. Admin only, and additionally subject to
    /// multisig quorum when access_control has a threshold above 1.
    ...
    /// Whitelist a stablecoin token. Admin only, and additionally subject to
    /// multisig quorum when access_control has a threshold above 1.
    ...
    pub fn fund_invoice(
        ...
        if fee > 0 {
            // A referrer recorded at listing time earns a share of the protocol
            // fee; the treasury only ever receives and accounts for the remainder.
            let referrer: Option<Address> = env
                .storage()
                .persistent()
                .get(&DataKey::Referrer(invoice_id));

            let referral_fee: i128 = match &referrer {
                Some(_) if config.referrer_split_bps > 0 => {
                    bps_of_normalized(fee, config.referrer_split_bps, token_decimals)?
                }
                _ => 0,
            };
            ...
            if referral_fee > 0 {
                ...
                token_client.transfer(&investor, &referrer, &referral_fee);
                events::referral_fee_paid(&env, invoice_id, &referrer, referral_fee);
            }
            ...
            if treasury_fee > 0 {
                ...
                treasury_client.collect_fee(&listing.token, &treasury_fee);
            }
        }
...
mod tests {
    ...
    fn deploy() -> TestEnv {
        ...
        // A real Stellar asset contract, so fee/referral transfers and balance
        // assertions exercise actual token movement rather than a stub address.
